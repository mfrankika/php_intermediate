...
// se enviou dados se esta logado 
if(isset($_SESSION['banco']) && empty($_SESSION['banco']) == false) {
	$id = $_SESSION['banco'];

	// consulta id enviado no bco 
	$sql = $pdo->prepare("SELECT * FROM contas WHERE id = :id");
	$sql->bindValue(":id", $id);
	$sql->execute();

	// se existir usuario com o id
	if($sql->rowCount() > 0) {

		// todos os dados da conta em $info
		$info = $sql->fetch();
	} else {
		// se nao achou apesar da sessao esta preenchida é usuario invalido
